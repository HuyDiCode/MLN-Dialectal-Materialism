...
const FloatingNav: React.FC = () => {
  ...
  const scrollToSection = (sectionId: string) => {
    const element = document.getElementById(sectionId === 'hero' ? 'root' : 'timeline-section');
    if (element) {
  ...
  return (
    <>
      {/* Navigation Dots */}
      <div className={`floating-nav ${isVisible ? 'floating-nav--visible' : ''}`}>
        <div className="nav-dots">
          <button
            className={`nav-dot ${activeSection === 'hero' ? 'nav-dot--active' : ''}`}
          ...
          <button
            className={`nav-dot ${activeSection === 'timeline' ? 'nav-dot--active' : ''}`}
